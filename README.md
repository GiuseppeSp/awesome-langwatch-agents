...
| 25 | [human-in-the-loop](agents/25-human-in-the-loop/) | act autonomously vs escalate to a human when the agent is unsure (self-reported confidence gate) | ✅ Shipped — closes Tier 4 (tools). "a human in the loop makes it safe" didn't hold: over 3 runs autonomous scored a stable **16/20 (80%)** and the checkpoint lifted it to 17/20 **once** and 0 the other two — `escalation_recall` **0.00–0.25**. The three actions that actually hurt (permanently deleting a file the user wanted archived) came at **0.90–1.00 confidence, `needs_human=false`** and were **never escalated in any run**, while the gate wasted an escalation on a *correct* DB drop (precision 0.50). HITL's accuracy gain equals exactly the errors it escalates, so its ceiling is escalation recall — and self-confidence is a low-recall, unstable (flips at temp 0) trigger, blind to the confident errors worth catching. Gate irreversible actions on risk, not the model's mood |
