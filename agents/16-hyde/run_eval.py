...
comparison, accuracy + retrieval hit-rate by category (clean vs poor), and the
paired vanilla -> hyde lift.

Usage:
    python run_eval.py                 # vanilla vs hyde, all rows
    python run_eval.py --mode hyde
...
import agent  # local
from evals import answer_correctness, retrieval_hit, hyde_lift
...
def run_one(row: Row, mode: str) -> dict:
    ...
    return {
        ...
        "answer": res.answer, "hypothetical": res.hypothetical, "llm_calls": res.llm_calls,
...
def _print_lift(results: list[dict]) -> None:
    ...
    if not all("vanilla" in m and "hyde" in m for m in by_q.values()):
        return
    print("\n=== Paired lift: vanilla -> hyde ===\n")
    helped = hurt = neutral = 0
    for q, m in by_q.items():
        d, w = m["vanilla"], m["hyde"]
        L = hyde_lift(d["correct"].passed, w["correct"].passed)
        if L.score == 1.0:
            helped += 1; tag = "  HELPED"
        elif L.score == 0.0:
            hurt += 1; tag = "    HURT"
        ...
        if tag:
            print(f"  [{d['category']:>5}] vanilla={'OK ' if d['correct'].passed else 'BAD'} "
                  f"hyde={'OK ' if w['correct'].passed else 'BAD'} (gold={d['gold']!r}){tag}")
            print(f"           q: {q[:60]}")
            print(f"           hypo -> {w['hypothetical'][:64]!r}")
    total = helped + hurt + neutral
    mean = (helped + neutral * 0.5) / total if total else 0.0
    print(f"\n  hyde_lift mean = {mean:.2f}  (helped {helped}, hurt {hurt}, neutral {neutral})")
